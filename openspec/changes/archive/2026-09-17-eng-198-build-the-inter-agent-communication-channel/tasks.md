...
- [x] 2.3 Verify both delivery paths render identically: the resident path through `#say`, and the dormant path through `#answerInLog`. Both already call `render()`; the task is a test that pins it rather than a change.
...
- [x] 3.4 Confirm the refusal still precedes any write — the guarantee `spawn`'s "so nothing was spawned" tail used to carry now rests on the check's position, and a test should hold it there.
...
- [x] 5.1 A parent and a child exchange messages through `send`, and the parent reads which child sent each one.
- [x] 5.2 Two children of one parent report; the parent's two messages are distinguishable by sender.
- [x] 5.3 An `await` answered after the agent's container went dormant carries the same mark it would have carried resident.
- [x] 5.4 A message from the human is read as from the human, not from an agent.
- [x] 5.5 A child whose report begins with `[substrate]` is read as that child's words, and the mark identifying it is still the one in mark position.
- [x] 5.6 A raw `send` frame from an agent whose record omits `send` is refused and delivers nothing.
- [x] 5.7 An unknown kind still answers "There is no capability named …" rather than a grant refusal.
- [x] 5.8 An agent granted neither `send` nor `await` still reports at turn end and still receives a message at a turn boundary.
- [x] 5.9 `await` raises the residency named in `keep`, and raises zero where `keep` is absent.
- [x] 5.10 Confirm each new guard fails when its source is mutated — the grant check, the kind check, and the escape. A source-level guard that no mutation can break is not being tested.

## 6. Notes and checks

- [x] 6.1 Rewrite `supervisor/AGENTS.md`'s "Two of the five request kinds read the caller's record" section to say what the rule now is, since this change resolves it.
- [x] 6.2 Add a short note there on the mark: what occupies mark position, that agent-authored content is escaped into it, and that a mid-message mark is deliberately not escaped.
- [x] 6.3 `npx tsc -p agent/tsconfig.json` and `npx vitest run --config agent/vitest.config.ts` — both clean. Nothing in CI covers `agent/`, so this is the only thing that will catch a regression.
- [x] 6.4 Run the container tier with Docker up, since `containers.test.ts` is where a record missing a capability surfaces as a hang rather than a failure.
- [x] 6.5 `openspec validate eng-198-build-the-inter-agent-communication-channel --strict`.
