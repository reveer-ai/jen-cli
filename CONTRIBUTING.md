...
1. **Check what landed** — root `AGENTS.md`, the six skills under `.claude/skills/` each carrying the stamp, `registry.yaml`, `.claude/settings.json`, and OpenSpec's own skills written beside jen's.
2. **Fill in the registry** — record a repository and a tracker project in `registry.yaml` by hand, as the README's third step tells an adopter to.
