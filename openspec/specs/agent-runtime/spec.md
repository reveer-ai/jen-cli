...
The runtime SHALL conduct its own reasoning loop: it decides when to call the model, dispatches the capabilities the model calls, appends their results, and decides when the exchange is over. It SHALL NOT delegate that control flow to a client library, a framework, or the coding assistant.
...
The distinction matters because the coding assistant is a capability the agent reaches for, not the agent's loop. If the assistant's loop were the agent's, reaching it through an adapter would mean nothing — the agent's behaviour would change wholesale with the assistant rather than one of its tools changing.
