...
Defines the sandbox primitive an agent works inside: creating one from an agent record with its credentials and workspace, running processes in it, destroying it, and releasing the workspace — the isolation it guarantees from the host and from other agents, its ephemeral-container and durable-workspace lifetime, its leak-freeness under repetition, and its defined failure modes.
