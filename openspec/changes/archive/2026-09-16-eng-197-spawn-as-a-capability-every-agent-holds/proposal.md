...
- Let a parent compose the child's name, charter, tools, and optionally the identifier of the model used for the child's own reasoning. The provider, endpoint, credential reference, sandbox environment, and workspace location inherit from the parent. The model override cannot redirect a credential to a new endpoint and does not select or configure a coding assistant.
