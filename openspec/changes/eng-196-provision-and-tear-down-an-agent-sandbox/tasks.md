...
- [x] 4.2 Implement `create`: ensure the agent's workspace volume, then start a container from the record's image with a trivial idle entrypoint, the volume mounted at the workspace path, and `jen.run` / `jen.agent` labels applied. The mount destination and the container's working directory are the same path and must stay so — see 4.13.
...
- [x] 4.13 Check `record.workspace` before composing it into the mount argument. The mount is written as `source:destination[:options]`, so a workspace of `/workspace:ro` moves the join rather than naming a path: the volume lands at `/workspace` read-only while the working-directory argument, handed the string whole, names `/workspace:ro` — a directory the runtime creates in the container's writable layer. Creation exits *zero*, processes start somewhere that looks right, and the agent's work is discarded at the next suspension. Require one absolute path with no `:`. Check the value rather than changing the flag: the workspace reaches the composed mount argument, the working-directory argument and every `exec`'s default location, and all three must name one place — `--mount type=volume,dst=…` preserves a `:` but reads `,` as a delimiter of its own, relocating the hazard and refusing paths the composed form carries. Note that this corrects 4.12's rule: a flag's argument is safe only where the value is handed over whole, never where this module builds the string around it.
...
- [x] 5.16 A workspace the mount argument would split is refused at creation and leaves nothing behind — `/workspace:ro`, `/workspace:/elsewhere`, and the non-absolute forms — with the unwind covered in both directions as in 5.15. Alongside it, assert the property the refusal protects: a process started without a location writes a file without naming a path, and that file is present after destroy and recreate. Record that this second test cannot catch the defect — once the value is refused there is no record left that produces a split mount, so it stays green with the refusal removed — because a passing persistence test otherwise reads as coverage of the bug.
