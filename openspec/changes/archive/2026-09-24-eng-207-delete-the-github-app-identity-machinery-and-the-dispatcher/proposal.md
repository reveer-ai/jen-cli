...
- `stage-conventions`: modify the record requirement, which hands refusal to "the dispatcher". Modify the announcement requirement to drop its dispatcher scenario and the marker. Modify the pull-request requirement's scenario about the pipeline running as an application. Modify the permissions requirement, which says a session decides each action on what it is: that was the deleted executor's `--permission-mode auto`, and is now the mode the invoker selects.
...
- `repo-scaffold`, `project-install`: modify the sentences that say a project's own permission entries are "in force in a dispatched run" and that `.claude/settings.json` is "what a dispatched run establishes trust for". Both also restate the executor's per-action judgment and its trust as givens; they become the session's permission mode and a project the assistant treats as trusted.
