...
- **An automated acceptance tier.** One run, real containers, the real runtime, asserting the structural criteria — depth ≥ 2, parallelism in isolated workspaces, messaging on the one path, `docker ps` empty under suspension, `kill -9` on the process group followed by a recovered tree that finishes, a killed child reported to its parent rather than hanging it, and nothing left behind at exit. Deterministic and free to re-run.
