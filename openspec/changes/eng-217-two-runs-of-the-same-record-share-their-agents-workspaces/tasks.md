...
- [x] 1.1 Split `slug` in `agent/sandbox/docker.ts` into `readable` and `digest` as above,
      leaving `slug` as their composition and its doc comment pointing at both.
- [x] 1.2 Give `workspaceName` the run parameter and the composition above, with a doc comment
      saying a workspace belongs to one agent **of one run** and why — the store is per-run and
      the volume must not disagree with it.
- [x] 1.3 Pass the driver's `#run` at both call sites: `create` (`docker.ts:291`) and
      `releaseWorkspace` (`docker.ts:384`).
- [x] 1.4 Confirm `releaseWorkspace(agentId)` keeps its signature and `agent/sandbox/index.ts`
...
- [x] 1.5 Confirm nothing else derives a volume name or sweeps volumes by label: `destroyAll`
...
- [x] 2.1 Add the regression to `agent/sandbox/docker.test.ts`: two drivers with different
...
- [x] 2.2 Release both workspaces explicitly in that test's `finally`. The file's `afterAll`
...
- [x] 2.3 Add the label-sweep assertion, which is the one that only means anything once the
...
- [x] 2.4 **Confirm 2.1 fails against the current code**, by stashing the `docker.ts` change
      and running it. A regression that passes before the fix is testing something else.
- [x] 2.5 Add the recovery case: the same run name and the same agent id, provisioned through a
...
- [x] 2.6 Run `npx vitest run --config agent/vitest.config.ts` in full, plus
...
- [x] 3.1 `agent/AGENTS.md`, under *Creation assumes one call at a time per agent*: replace the
      ...
      real.
- [x] 3.2 `agent/LIVE-PASS.md`, under *Afterwards: the workspaces are yours*: remove the
...
- [x] 3.3 Confirm the findability block in `agent/AGENTS.md` under *A run's workspaces outlive
...
- [x] 3.4 Judge whether anything here clears the bar for a *new* note. Probably not: the
      guarantee belongs in `workspaceName`'s doc comment and in the spec, and a note repeating
      them earns nothing.

## 4. Close the change out

- [x] 4.1 `npx openspec validate eng-217-two-runs-of-the-same-record-share-their-agents-workspaces --strict`.
- [x] 4.2 `npm run typecheck`.
- [x] 4.3 Check the delta against the main specs once more at requirement granularity, per
...
- [x] 4.4 **No migration.** Existing `jen-workspace-<agent>` volumes on a developer's machine
