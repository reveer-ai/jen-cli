...
Add `spawn` and `stop` declarations to the existing `SUPERVISED` registry in `agent/runtime/main.ts`, using `supervised()` to raise the named request over the correlated JSON-lines channel. Their JSON Schemas become function-tool declarations in the SDK's chat-completions request. The model decides from its context whether to return a `spawn` tool call; jen's generic loop dispatches only what the model called, and no code decides when spawning is appropriate. The registry determines availability from `record.tools`. Neither `Runtime`, its dispatch loop, nor the protocol gains a spawn branch. A record without `spawn` cannot call it through the model tool surface; a descendant explicitly granted `spawn` uses the same declaration as its parent. The SDK's automatic tool-running helpers are not used.
...
The `spawn` input is an object with required non-empty `name` and `charter`, optional `tools` (array of capability names), optional `model` (a non-empty model identifier string), and optional `opening` (the first parent message). The caller cannot supply `id`, `parent`, `credentials`, `environment`, `workspace`, `provider`, or `baseURL`. The supervisor constructs the sole `AgentRecord`: assigns an id, sets `parent` to the caller, copies the parent's environment, workspace *path inside its own isolated volume*, and credential references, and copies the parent's model configuration with only its `model` field replaced when requested.

A model override changes only which model the child uses for its own reasoning at the inherited provider endpoint. `provider`, `baseURL`, and `credential` remain the parent's; the supervisor validates the non-empty identifier and resulting record before storing or provisioning it. This prevents a spawn request from redirecting the inherited credential to an arbitrary endpoint. The coding assistant is a separate capability and subprocess (ENG-211), potentially using a different credential reference; choosing a reasoning model here neither selects that assistant nor changes its credential. No credential value enters a request or record.

Alternative: accept a whole `AgentRecord` from the caller and overwrite `id`/`parent`. That invites caller-controlled fields the supervisor does not intend to grant, duplicates construction policy on both sides of the channel, and weakens the guarantee that parentage and credentials are supervisor-derived. Alternative: accept a complete model configuration. That permits an arbitrary `baseURL` to receive an inherited credential, so a confused agent could redirect a secret while appearing merely to choose a model.
...
This is capability physics, not a role policy: it says what a record can authorize, not what a reviewer or accountant should decide. At the request boundary the supervisor also checks that the caller's record grants `spawn` (and, for dismissal, `stop`); a raw request frame cannot bypass record selection. It checks the child tool subset there too, because that is where the new record is made and a record that silently drops or widens tools would lie to the parent about what it created. The supervisor validates the requested model and other fields rather than relying on model-facing JSON Schema alone; schema is a guide to the model, not a trust boundary.
...
- **A model override redirects an inherited credential** → Accept only a model identifier string and inherit the provider, endpoint, and credential unchanged; reject malformed or empty identifiers before writing.
