...
**Anyone who installed an earlier version keeps the entries jen wrote.** `jen update` never rewrites that file, so nothing removes them for you, and keeping them costs nothing directly — be aware only of what a broad entry exempts. Three of the four npm entries jen shipped stop existing regardless: `auto` discards package-manager run commands from `permissions.allow` on entry.
